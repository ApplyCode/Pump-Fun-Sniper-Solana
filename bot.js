...
const {
    ...
    TransactionMessage, // for jito
    VersionedTransaction, // for jito
    PublicKey,
...
const PRIVATE_KEY_HEX = fs.readFileSync('privkey.txt').toString();
const pvk = Uint8Array.from(Buffer.from(PRIVATE_KEY_HEX, 'hex'));
...
const NET_NAME = 'mainnet-beta';
const CUSTOM_RPC_URL = `https://api.${NET_NAME}.solana.com`;
...
const loadSettings = () => {
    ...
    return {
        auto: false,
        buyAmount: 0.1,
        jitoTipAmount: 1000, // 1,000,000,000 lamports = 1 sol
...
const {checkValidation} = require('./jito-utils')
...
const getCoinData = async (mint) => {
    console.log("getCoinData > mint: ", mint);
...
const getKingOfTheHillCoin = async () => {
    ...
    // {
    //     const beforeProjects = await getBeforeProjects(coin.creator);
    //     coin.beforeProjects = beforeProjects;
    // }
...
const getBeforeProjects = async (developerAddress) => {
    const response = await fetch(`https://frontend-api.pump.fun/balances/${developerAddress}?limit=50&offset=0&minBalance=-1`);
    // if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    if (!response.ok) {
        return [];
    }
...
const checkIfAuthoritiesRevoked = async (mintAddress) => {
    const connection = new Connection(CUSTOM_RPC_URL || clusterApiUrl(NET_NAME), 'confirmed');
...
const displayCoinDetails = async (connection, coin) => {
    ...
    // console.log("coin.creator: ", coin.creator);
    ...
    // I mean like when the developer on pump.fun has had previous projects of there’s hit ray, and so when they have a new coin it will likely send as well. Sniper these and it’s big money

    console.log(`Previous projects: `);
    for (const project of beforeProjects) {
        console.log(`symbol: ${project.symbol}, market_cap: ${project.market_cap}`);
    }
    if (beforeProjects.length < 1)
    {
        console.log("none");
    }
    ...
    if (action === 'buy') {
        ...
        const payer = Keypair.fromSecretKey(pvk);
...
const jitoInit = async () => {
    const authDecodedKey = new Uint8Array(
        JSON.parse(fs.readFileSync('auth.json').toString()),
      );

    const authKeypair = Keypair.fromSecretKey(authDecodedKey);
    jito_client = (0, searcher_1.searcherClient)(blockEngineUrl, authKeypair);

    const _tipAccount = (await jito_client.getTipAccounts())[0];
    console.log('\n\nTip account:', _tipAccount);
    checkValidation(_tipAccount);
...
const buyCoin = async (connection, payer, coinData, solIn, priorityFeeInSol = 0.001, slippageDecimal = 0.25) => {
    try {
        ...
        try {
            ...
            // console.error(`#1 connection.getAccountInfo error:`, error);
            // console.error(`connection.getAccountInfo error`);
        }
        // console.log("buyCoin() #2");
        const instructions = [];
        if (!tokenAccountInfo) {
            instructions.push(
        ...
        const keypair2 = Keypair.fromSecretKey(pvk);
        ...
        const respJito = await jito_conn.getLatestBlockhash('processed');
        // console.log("### respJito: ", respJito);

        const tx = new Transaction();
        tx.recentBlockhash = respJito.blockhash;
        tx.lastValidBlockHeight = respJito.lastValidBlockHeight;
        // console.log("### tx.recentBlockhash: ", tx.recentBlockhash);
        // console.log("### tx.lastValidBlockHeight: ", tx.lastValidBlockHeight);
        ...
        maybeBundle = maybeBundle.attachTip(
            keypair2,
            settings.jitoTipAmount,
            tipAccount,
            respJito.blockhash,
            respJito.lastValidBlockHeight
        );
        ...
        try {
            const resp2 = await jito_client.sendBundle(maybeBundle);
            // console.log('resp:', resp2);
            console.log('Buy transaction done!');
            return tokenOut;
        } catch (e) {
            // console.log('error:', e);
            return null;
...
const setSettings = async () => {
    ...
    const answers = await inquirer.default.prompt([
        ...
        {
            type: 'confirm',
            name: 'require_before_projects',
            message: 'Require Before Projects of Creator:',
            default: settings.require_before_projects
        },
        {
            type: 'confirm',
        ...
        },
        {
            type: 'input',
            name: 'jitoTipAmount',
            message: 'Set Jito Tip amount (lamports):',
            default: settings.jitoTipAmount,
            validate: (value) => !isNaN(value) && value > 0
        },
        {
            type: 'confirm',
    ...
    settings = {
        ...
        require_before_projects: answers.require_before_projects,
        require_revoked_authority: answers.require_revoked_authority,
        auto: answers.auto,
        buyAmount: parseFloat(answers.buyAmount),
        jitoTipAmount: parseFloat(answers.jitoTipAmount),
...
const filterCoins = async (coins) => {
    ...
    for (const coin of coins) {
        const beforeProjects = await getBeforeProjects(coin.creator);
        coin.beforeProjects = beforeProjects;
        // console.log("settings.min_usd_market_cap: ", settings.min_usd_market_cap);
        // console.log("coin.reply_count: ", coin.reply_count);
        // console.log("settings.min_reply_count: ", settings.min_reply_count);
        if (
            ...
            (!settings.require_before_projects || coin.beforeProjects && coin.beforeProjects.length > 0)
            ) {
...
const buyLatestCoin = async (connection, payer) => {
    while (true) {
        ...
        // console.log(`#5 Buying ${settings.buyAmount} SOL worth of ${coin.symbol}...`);
...
const buyLatestCoins = async (connection, payer) => {
    ...
    while (settings.auto && autoBuyRunning) {
        ...
        // console.log(`#4 Buying ${settings.buyAmount} SOL worth of ${coin.symbol}...`);
...
const buyCoinByContract = async (connection, payer) => {
    ...
    // console.log(`#3 Buying ${settings.buyAmount} SOL worth of ${coin.symbol}...`);
    ...
    try {
        ...
        while (true) {
            ...
            if (action === 'buy_latest_coin') {
                ...
                // console.log(`#2 Buying ${settings.buyAmount} SOL worth of ${coin.symbol}...`);
                ...
                while (true) {
                    const selectedCoin = await displayLatestCoins(connection, 10); // Fetch more coins
                    if (!selectedCoin) break;

                    const coinAction = await displayCoinDetails(selectedCoin);
                    if (coinAction === 'buy') {
                        // console.log(`#1 Buying ${settings.buyAmount} SOL worth of ${selectedCoin.symbol}...`);
